...
test.describe('Patients - E2E', () => {
  ...
  test.describe('Patients List', () => {
    ...
    test('should display patient count', async ({ page }) => {
      ...
      const countText = await page.locator('text=/\\d+ paciente/i').first().textContent()
  ...
  test.describe('Create Patient', () => {
    ...
    test('should create valid patient', async ({ page }) => {
      ...
      await page.locator('#curp').fill(`TEST${timestamp.toString().slice(0, 10)}HNLRN01`)
    ...
    test('should validate CURP format', async ({ page }) => {
      ...
      const curpError = page.locator('text=/CURP inválido|formato/i')
      if (await curpError.count() > 0) {
        expect(true).toBe(true)
      }
    })
    ...
    test('should validate date of birth', async ({ page }) => {
      ...
      const dateError = page.locator('text=/fecha|menor|nacimient/i')
    ...
    test('should view patient details', async ({ page }) => {
      ...
      if (await firstRow.count() > 0) {
        ...
        if (await patientLink.count() > 0) {
          ...
          await expect(page.getByText(/Datos del Paciente|Información del Paciente/i)).toBeVisible({ timeout: 5000 })
        }
      }
    })
    ...
    test('should display patient personal information', async ({ page }) => {
      ...
      if (await firstRow.count() > 0) {
        ...
        if (await patientLink.count() > 0) {
          ...
          await expect(page.getByText(/Nombre|Fecha de nacimiento|Sexo|Teléfono/i)).toBeVisible()
        }
      }
    })
  ...
  test.describe('Pagination', () => {
    test('should display pagination', async ({ page }) => {
      ...
      const pagination = page.locator('nav[aria-label="pagination"], .pagination, nav button')
    ...
    test('should navigate to next page', async ({ page }) => {
      ...
      const nextButton = page.locator('button[aria-label="next"], .next, text=/Siguiente/i').first()
