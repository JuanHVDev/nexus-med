...
import { GET, PUT, DELETE } from '@/app/api/lab-orders/[id]/route'
...
describe('Lab Orders [id] API Route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET /api/lab-orders/[id]', () => {
    it('should return lab order by id', async () => {
      ...
      const response = await GET(new NextRequest('http://localhost/api/lab-orders/1'), {
    ...
    it('should return 404 when lab order not found', async () => {
      ...
      const response = await GET(new NextRequest('http://localhost/api/lab-orders/999'), {
  ...
  describe('PUT /api/lab-orders/[id]', () => {
    it('should update lab order successfully', async () => {
      vi.mocked(prisma.labOrder.findFirst).mockResolvedValue(mockLabOrder as any)
      ...
      const request = new NextRequest('http://localhost/api/lab-orders/1', {
        method: 'PUT',
    ...
    it('should return 404 when lab order not found', async () => {
      ...
      const request = new NextRequest('http://localhost/api/lab-orders/999', {
        method: 'PUT',
        body: JSON.stringify({ status: 'COMPLETED' })
      })
  ...
  describe('DELETE /api/lab-orders/[id]', () => {
    it('should delete lab order successfully', async () => {
      vi.mocked(prisma.labOrder.findFirst).mockResolvedValue(mockLabOrder as any)
      vi.mocked(prisma.labOrder.delete).mockResolvedValue(mockLabOrder as any)

      const response = await DELETE(new NextRequest('http://localhost/api/lab-orders/1'), {
        params: Promise.resolve({ id: '1' })
      })
      const data = await response.json()

      expect(response.status).toBe(200)
      expect(data.message).toBe('Lab order deleted')
    })
    ...
    it('should return 404 when lab order not found', async () => {
      ...
      const response = await DELETE(new NextRequest('http://localhost/api/lab-orders/999'), {
        params: Promise.resolve({ id: '999' })
      })
